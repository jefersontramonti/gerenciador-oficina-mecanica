...
export const ForgotPasswordPage = () => {
  ...
  const onSubmit = async (data: ForgotPasswordFormData) => {
    try {
      ...
      setApiError(err.response?.data?.message || 'Erro ao processar solicitação. Tente novamente.');
    } finally {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-gray-900 to-black px-4 py-8">
      ...
      <div className="w-full max-w-md rounded-xl border border-gray-700 bg-gray-800 p-8 shadow-2xl">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-white">Esqueceu sua Senha?</h1>
          <p className="mt-2 text-gray-400">
            Não se preocupe, enviaremos instruções para você
          </p>
        </div>
        ...
        {/* Mensagem de Sucesso */}
        {successMessage && (
          <div className="mb-6 flex items-start gap-2 rounded-lg border border-green-800 bg-green-900/20 p-3 text-sm text-green-400">
        ...
        {!successMessage ? (
          ...
          /* Após sucesso - Botão voltar para login */
          <div className="space-y-4">
            <div className="rounded-lg border border-blue-800 bg-blue-900/10 p-4 text-sm text-gray-300">
              <p className="mb-2 font-medium text-blue-400">Próximos passos:</p>
            ...
            <Link
              to="/login"
              className="block w-full rounded-lg border border-gray-600 bg-gray-700 px-4 py-3 text-center font-medium text-white transition-colors hover:bg-gray-600"
            >
              Voltar para Login
            </Link>
          </div>
